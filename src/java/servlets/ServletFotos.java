...
@WebServlet(name = "ServletFotos", urlPatterns = {"/ServletFotos"})
                ...
                while( it.hasNext() )
                {
                    ...
                    if( !item.isFormField() )
                    {
                        if( item.getSize() > 0 )
                        {
                            ...
                            File archivo = new File(getServletContext().getRealPath("images/empleados"), nombre);
                            item.write(archivo);

                            if ( archivo.exists() )
                            {
                                String rutaFoto = "images/empleados" + "/" + nombre;
